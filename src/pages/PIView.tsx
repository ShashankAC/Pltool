...
type StoriesBreakUp = {
    id: number;
    value: number;
    label: 'BUG' | 'IMPROVEMENT' | 'FEATURE' | 'MAINTENANCE'
}
...
function PIView() {
    ...
    const [storiesBreakUp, setStoriesBreakup] = useState<StoriesBreakUp[]>([]);
    ...
    useEffect(() => {
        let storiesBreakUpList: StoriesBreakUp[] = [
            {
                id: 0,
                value: 0,
                label: 'BUG'
    ...
    const prepareSprintwiseStorytypeBreakup = (stories: Story[], sprints: Sprint[]) => {
        const result: {data: Array<number>}[] = [];
        ...
        for (let i = 0; i < sprints.length; i++) {
            for (let j = 0; j < stories.length; j++) {
                for (let k = 0; k < Object.keys(STORYTYPES).length; k++) {
                    if (stories[j].type === Object.keys(STORYTYPES)[k] && stories[j].sprints.indexOf(sprints[i].name) !== -1) {
                        result[k].data[i] +=1;
                    }
                }
            }
    ...
    return (
        <Box sx={{ margin: '10px 0px 10px 10px', width: '98%' }}>
            ...
            <Typography variant="h6" gutterBottom>
                Start date: {dayjs(startDate).format('DD/MM/YYYY')}
            </Typography>
            <Typography variant="h6" gutterBottom>
                End date: {dayjs(endDate).format('DD/MM/YYYY')}
            </Typography>
            <Typography variant="h6" gutterBottom>
                Team size: {teamMembers?.length || 0}
            </Typography>
            <Typography variant="h6" gutterBottom>
                Number of stories: {stories?.length || 0}
            </Typography>
            <Typography variant="h6" gutterBottom>
                Working hours per day: {hoursPerDay || 0}
            </Typography>
            <Typography variant="h6" gutterBottom>
                Number of working days: {getNoOfWorkingDays(dayjs(startDate), dayjs(endDate), holidays)}
            </Typography>
            <Typography variant="h6" gutterBottom>
                Number of holidays: {getNumberOfHolidays(dayjs(startDate), dayjs(endDate), holidays)}
            </Typography>
            <Typography variant="h6" gutterBottom>
                Required story points: {totalStoryPoints}
            </Typography>
            <Typography variant="h6" gutterBottom>
                Possible story points: {possibleStoryPoints}
            </Typography>
            {totalStoryPoints > possibleStoryPoints ? (
                <Alert severity="warning" variant="filled">One or more team members are overloaded.</Alert>
            ): null}
            ...
            <Typography variant="h6">PI Breakup By Story Type</Typography>
            <PieChart
